...
def upload_data(creds, rec_folder, upload_folder):
    ...
    for file in files:
        print("FILE ===> " + file)
        filename = file.replace('./NHK2_', '実践ビジネス英語_')
        ...
        service.files().create(body=file_metadata,
                                    media_body=media,
                                    fields='id').execute()
        queryword = "name='" + filename + "'"
        print("queryword ===> " + queryword)

        results = service.files().list(q=queryword, fields="nextPageToken, files(id, name)").execute()
        ...
        if not items:
            ...
            for item in items:
                ...
                uploaded_fileid = str(item['id'])
                print("uploaded_fileid ===> " + uploaded_fileid)
                ...
                user_permission = {
                                    'type': 'user',
                                    'role': 'writer',
                                    'emailAddress': 'yumoturismo@gmail.com'
                                    }
                batch.add(service.permissions().create(
                                        fileId=uploaded_fileid,
